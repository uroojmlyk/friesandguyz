...
//       {/* ✅ WISHLIST BUTTON - ADD THIS */}
//       <button
//         onClick={() => inWishlist ? removeFromWishlist(item.id) : addToWishlist(item)}
//         className="absolute top-2 right-2 text-3xl hover:scale-110 transition-transform z-10"
//       >
//         {inWishlist ? '❤️' : '🤍'}
//       </button>
...
const MenuCard = ({ item, onAddToCart }) => {
  ...
  // ✅ Debug - Check if item is being passed correctly
  console.log('MenuCard item:', item);

  const handleWishlistClick = (e) => {
    e.preventDefault();
    e.stopPropagation();
    console.log('Wishlist clicked for item:', item);
    
    if (inWishlist) {
      removeFromWishlist(item.id);
      console.log('Removed from wishlist:', item.id);
    } else {
      // ✅ Make sure we're passing the complete item object
      addToWishlist({
        id: item.id,
        name: item.name,
        price: item.price,
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2 right-2 text-3xl hover:scale-110 transition-transform z-20 ${
