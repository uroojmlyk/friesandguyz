...
const Menu = () => {
  ...
  const handleCheckout = () => {
    console.log('🟢 Checkout clicked - opening payment');
    if (cart.length === 0) {
            ...
            <div className="relative">
              ...
              {getTotalItems() > 0 && (
                <div className="absolute -top-2 -right-2 bg-[#FFD966] w-8 h-8 rounded-full border-3 border-black shadow-[4px_4px_0_black] flex items-center justify-center animate-bounce-slow">
                  <span className="font-['Fredoka'] font-bold text-lg">{getTotalItems()}</span>
                </div>
              )}
        ...
        {showCart && (
          <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white/95 backdrop-blur-xl border-l-4 border-black shadow-[-20px_0_0_black] z-50 p-6 animate-slide-left overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowCart(false)} className="text-3xl hover:scale-90 transition-transform">✕</button>
            </div>
            ...
            {cart.length === 0 ? (
              ...
              <>
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-center gap-4 bg-[#FFE5D9] p-4 rounded-[30px] border-3 border-black shadow-[6px_6px_0_black] animate-slide-right">
                      ...
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 bg-white rounded-full border-2 border-black text-xl font-bold hover:scale-90">-</button>
                        <span className="font-['Fredoka'] font-bold text-lg w-6 text-center">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 bg-white rounded-full border-2 border-black text-xl font-bold hover:scale-90">+</button>
                      </div>
                    </div>
                  ))}
                ...
                <div className="bg-[#FFD966] p-6 rounded-[40px] border-4 border-black shadow-[8px_8px_0_black] mb-6">
                  ...
                  {/* CHECKOUT BUTTON - THIS OPENS PAYMENT POPUP */}
                  <button
                    onClick={handleCheckout}
                    className="w-full bg-black text-white font-['Fredoka'] text-xl font-bold py-4 px-6 rounded-[40px] border-4 border-black shadow-[6px_6px_0_white] hover:scale-95 transition-all duration-200 flex items-center justify-center gap-3"
                  >
          ...
          {categories.map((category) => (
            <button
              ...
              className={`font-['Fredoka'] text-sm sm:text-base lg:text-lg font-bold px-5 sm:px-6 py-2.5 sm:py-3 rounded-full border-3 sm:border-4 border-black shadow-[6px_6px_0_black] hover:shadow-[3px_3px_0_black] hover:scale-95 transition-all duration-200 ${
                activeCategory === category ? 'bg-[#FFB5B5] text-black' : 'bg-white text-black hover:bg-[#FFD966]'
              }`}
            >
              {category === 'all' ? `🍽️ ${t?.menu?.allItems || 'All Items'}` : category}
              {category !== 'all' && (
                <span className="ml-2 text-sm opacity-75">({menuItems.filter(item => item.category === category).length})</span>
              )}
            </button>
          ))}
        </div>
